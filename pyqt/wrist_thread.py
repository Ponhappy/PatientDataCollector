...
class WristDataThread(QThread):
    ...
    def __init__(self, serial_port, baudrate=38400, parent=None):
    ...
    def run(self):
        save_wrist_pulse(self.csv_filename,self.serial_port)
        # try:
        #     ser = serial.Serial(self.serial_port, self.baudrate, timeout=1, bytesize=8, stopbits=1, parity='N')
        #     print(f"手腕串口 {self.serial_port} 已打开。")
        # except serial.SerialException as e:
        #     print(f"无法打开手腕串口 {self.serial_port}: {e}")
        #     return
        ...
        #     if csv_writer:
        #         csv_file.close()
        #     print(f"手腕串口 {self.serial_port} 已关闭。")
